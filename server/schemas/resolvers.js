...
const resolvers = {
    Query: {
        getClients: async (parent, { }, context) => {
            if (!context.user) {
                throw new GraphQLError('Must be an authorized user to view this page.', {
                  extensions: {
                    code: 'UNAUTHENTICATED',
                    myExtension: 'foo', //not sure what the myExtension property does...
                  },
                });
            }
            try {
    ...
    Mutation: {
        ...
        addProjectImage: async (_, { projectId, downloadURL }, context) => {
            ...
            if (!context.user) {
              throw new GraphQLError('Must be an authorized user to add project images', {
                extensions: {
                  code: 'UNAUTHENTICATED',
                  myExtension: 'foo',
                },
            ...
            if (!mongoose.Types.ObjectId.isValid(projectId)) {
                throw new ApolloServerErrorCode.BAD_USER_INPUT('Invalid projectId');
            }
            ...
            if (!project) {
                throw new ApolloServerErrorCode.BAD_USER_INPUT('Project not found');
            }
        ...
        addHomePhoto: async (_, { clientId, downloadURL }, context) => {
            ...
            if (!context.user) {
                throw new GraphQLError('Must be an authorized user to add home photos.', {
                  extensions: {
                    code: 'UNAUTHENTICATED',
                    myExtension: 'foo',
                  },
            ...
            if (!mongoose.Types.ObjectId.isValid(clientId)) {
                throw new ApolloServerErrorCode.BAD_USER_INPUT('Invalid clientId');
            }
            const client = await Client.findById(clientId);
            if (!client) {
                throw new ApolloServerErrorCode.BAD_USER_INPUT('Client not found');
            }
